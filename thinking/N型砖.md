...
0. 选股范围：沪深A股, 市值大于50亿
1. N型拉升的股票 : 前30交易日内有量价齐升的拉升波段，近期处于拉升后的回调阶段，剔除前期有S1或者大风车的股票, 也要剔除前期有跳空涨停的股票
2. 出现第一块红砖，并且红砖前一个交易日相对缩量较好的。
2.1 金砖选股
...
- 账户总金额10w, 单支股票最多买入5w
- 选出下大上小(砖大,涨幅小)前两只股票买入(不要选涨停的股)

3. 止损和止盈

- 选股后T+1 开盘买入(按分钟线买入)
- 持仓过程中,红砖变绿砖则直接清仓
- 如果持仓盈超过5%,则持股直到红砖变绿砖清仓
- 止损位为-2%或两日内不拉起来就走；
- 买入后只要有2%以上的涨幅,则卖出1/4, 后续每天涨幅超过2%则继续卖出1/4, 直到全部卖出(同一天不能超过2次这样的操作,除非是涨停则清仓)
...
STICKLINE(REF(砖型图,1)>砖型图,砖型图,REF(砖型图,1),3,0),COLORGREEN;
红持绿空:=0,COLORGREEN;
AA:=(REF(砖型图,1)<砖型图);
BB:=REF(砖型图,1)>砖型图;
...
```js
{公式名称：N型上涨-缩量深回调+窄幅整理+市值筛选}
{参数（可自行调整）}
N:=20;          {近期搜索周期：高点发生在最近20天内}
ZF:=15;         {前期上涨最低涨幅（%）}
LB:=1.5;        {上涨期均量相对前期均量的倍数}
HD:=15;         {最小回调幅度（%）：当前股价从高点回落需超过此幅度}
ZD:=10;          {最大振幅限制（%）：最近5日单日振幅不超过此值}
SZ:=50;         {最小总市值（亿元）}

{1. 识别近期高点：最近N日的最高价，且高点出现在5天前以上}
近期高点:=HHV(H,N);
高点距今日:=HHVBARS(H,N);
高点条件:=高点距今日>=5 AND 高点距今日<N;

{2. 计算上涨起点与涨幅：高点前方10日内的最低点为A}
上涨前低:=REF(LLV(L,10), 高点距今日+1);
涨幅:=(近期高点-上涨前低)/上涨前低*100;
涨幅条件:=涨幅>=ZF AND 涨幅<50;

{3. 上涨阶段量价齐升：取高点前5日起的10日均量，与更早的10日均量比较}
上涨段均量:=REF(MA(V,10), 高点距今日+5);
前期均量:=REF(MA(V,10), 高点距今日+15);
量能条件:=上涨段均量 > 前期均量 * LB;

{4. 上涨非跳空拉涨：最近20天内无跳空涨停（排除无量一字板拉升）}
无量拉升条件:=COUNT(L>REF(H,1) AND C/REF(C,1)>1.095, 20)=0;

{5. 高点未放量：高点当日成交量，小于上涨期间最大成交量的90%}
上涨最高量:=REF(HHV(V,10), 高点距今日+1);
高点不放量:=REF(V, 高点距今日) < 上涨最高量 * 0.9;

{6. 当前处于回调：收盘价低于近期高点，且高于上涨起点}
当前回调:=C < 近期高点 AND C > 上涨前低;

{7. 回调缩量：当前5日均量小于上涨段均量的80%}
当前5均量:=MA(V,5);
回调缩量:=当前5均量 < 上涨段均量 * 0.8;

{8. 回调幅度超过HD%（默认10%）}
回调幅度:=(近期高点-C)/近期高点*100;
回调深幅:=回调幅度>=HD;

{9. 最近5个交易日单日振幅均不超过ZD%}
单日振幅:=(H-L)/REF(C,1)*100;
窄幅整理:=EVERY(单日振幅<=ZD, 5);

{10. 新增：总市值大于SZ亿元（默认50亿）}
总市值:=FINANCE(40)/100000000;  {总市值单位：亿元}
市值条件:=总市值>SZ;

{综合选股条件}
XG:高点条件 AND 涨幅条件 AND 量能条件 AND 无量拉升条件 AND 高点不放量 
    AND 当前回调 AND 回调缩量 AND 回调深幅 AND 窄幅整理 AND 市值条件;
```
