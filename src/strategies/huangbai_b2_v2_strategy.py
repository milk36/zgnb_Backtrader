...
核心逻辑（在B2基础上新增30日B1频次过滤）：
1. 大盘MACD多头时才买入（空头只卖不买）
2. 周线多头空间过滤
3. 最近30个交易日内至少有2个B1信号，且间隔≥5个交易日
...
和30日B1频次过滤，复用 PortfolioSimulator 的标准六级退出。
"""
...
from config import (
    ...
    B2_V2_B1_LOOKBACK, B2_V2_B1_MIN_COUNT, B2_V2_B1_MIN_GAP,
    MARKET_INDEX_CODE,
)
...
def _compute_all_bar_signals(C, H, L, O, V, dates, params, capital_shares=None):
    """B2_V2: 30日B1频次过滤 + 前日B1 + 当日倍量柱

    包装V4的_compute_all_bar_signals，修改：
    1. 新增30日B1频次过滤（至少2次，间隔>=5天）
    2. b1 → 前一天b1 & 当天倍量柱 & 30日B1频次通过 (B2_V2入场条件)
    3. dongneng_recent → 全True (不做动能过滤)
    """
    ...
    # B2_V2入场条件：前一天B1 & 当天倍量柱 & 30日B1频次通过
    signals["b1"] = prev_b1 & beiliangzhu & b1_count_ok
    ...
    signals["b1_count_ok"] = b1_count_ok
    ...
    # B2_V2排序：缩量升序优先（不设b2_sort_primary，复用默认shrink_score排序）

    return signals


def _compute_signals(C, H, L, O, V, dates, params):
    """计算最新 bar 的 B2_V2 信号结果（用于实时扫描）"""
    ...
    return {
        ...
        "b1_count_ok": bool(all_bars["b1_count_ok"][i]),
    }
...
def _scan_one(code, params, skip_weekly, market_macd_ok=True):
    """扫描单只股票（B2_V2: 30日B1频次 + 前日B1 + 当日倍量柱）"""
...
def scan_all(stock_type="main", skip_weekly=False,
    ...
    # B2_V2排序：缩量升序优先，次按涨幅接近4.5%
    results.sort(key=lambda x: (x["shrink_score"], abs(x.get("daily_pct", 0) - 4.5)))
...
def _scan_one_all_bars(code, params):
    ...
    try:
        ...
        if signals is not None:
            ...
            # B2_V2 T+1买入：将入场信号和排序字段前移1天（不环绕）
            for key in ("b1", "shrink_score", "vol_expand_ok",
                ...
                if arr is not None and len(arr) > 1:
                    ...
                    signals[key] = shifted
            # 不前移 b1_count_ok（它描述的是当日的B1频次状态）
...
def preload_all_signals(start="2024-01-01", end="2025-12-31",
    ...
    """B2_V2预加载：大盘MACD过滤 + 30日B1频次 + 前日B1 + 当日倍量柱
    ...
    codes = _get_all_codes(tdxdir)
    total = len(codes)
    print(f"预加载 {total} 只A股信号(B2_V2倍量柱)... (workers={max_workers or 'auto'})")
