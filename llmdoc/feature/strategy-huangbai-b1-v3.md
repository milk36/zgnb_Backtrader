...
`_compute_all_bar_signals()` 返回字典包含 `open`、`volume`、`avg_amount_20`（20日成交额均值，流通市值代理指标）和 `vol_expand_ok`（前期放量过滤，含S1/大风车排除）字段，分别供 K 线图模块绘制和 PortfolioSimulator 选股过滤/排序使用。

### 突然放巨量阴线检测

与 V1/VV2 相同，独立于 vol_expand_ok 过滤链：
- **定义**：`V > REF(V,1)*3` 且 `V > MA(V,20)*3` 且 `C < O` 且 `(O-C)/O > 0.03`
- **选股过滤**：60日内出现则剔除（`no_huge_vol_bearish`）
- **持股退出**：PortfolioSimulator 中巨量阴线清仓（仅次于止损）
- 仅在 `_compute_signals()` 和 `_compute_all_bar_signals()` 中实现，不在 `indicators()` 中
...
- `config.py` — 策略参数配置（含 `HUANGBAI_VOL_EXPAND_PERIOD`、`HUANGBAI_VOL_EXPAND_MIN`、`HUANGBAI_SURGE_PRICE_PCT`、`HUANGBAI_SURGE_VOL_RATIO`、`HUANGBAI_S1_PERIOD`）
...
- 止盈止损逻辑与 V1/V2 完全一致：涨停优先于中阳，涨停不受中阳标记限制；中阳卖1/3 需满足当日上涨（`price > prev_close`）且累计盈利达标，通过 `_mid_yang_triggered` 标记确保每次持仓仅触发一次
- S1/大风车/长上影线排除仅在 `_compute_all_bar_signals()` 中实现（V3 组合模拟路径），V3 的 `indicators()` 和 `_compute_signals()` 中未实现此过滤
- 长上影线检测（`_long_upper_shadow`）：C>=HHV(C,20)*0.97 + 上影线>3% + 上影线>实体*2 + 量>前日*1.3，四条件AND
