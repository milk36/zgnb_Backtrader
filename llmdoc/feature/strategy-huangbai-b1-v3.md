...
`_compute_all_bar_signals()` 返回字典包含 `open`、`volume`、`avg_amount_20`（20日成交额均值，流通市值代理指标）和 `vol_expand_ok`（前期放量过滤）字段，分别供 K 线图模块绘制和 PortfolioSimulator 选股过滤/排序使用。
...
- `config.py` — 策略参数配置（含 `HUANGBAI_VOL_EXPAND_PERIOD`、`HUANGBAI_VOL_EXPAND_MIN`、`HUANGBAI_SURGE_PRICE_PCT`、`HUANGBAI_SURGE_VOL_RATIO`）
