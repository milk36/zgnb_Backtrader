...
1. `run()` 逐日遍历交易日历：
   ...
   - `vol_expand_ok` 前期放量过滤：要求近期有放量上涨支撑，且排除缩量快速拉升（涨幅>15%且量比<0.7）的股票
   - 每日：检查持仓卖出条件（止损→T+N→盈利100%清仓→半仓持股模式→涨停卖半→中阳卖1/3(需当日上涨)）
   - 记录每日权益曲线
...
6. **中阳卖1/3**：当日收盘价 > 前一日收盘价（`daily_up`）且涨幅>=主板5%/科创板10%；通过 `mid_yang_triggered` 标记确保每次持仓仅触发一次（避免中阳持续期间每天重复卖出）；触发后标记 `mid_yang_triggered=True`；剩余<=半仓进入条件4
