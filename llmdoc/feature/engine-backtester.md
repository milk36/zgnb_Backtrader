...
4. **盈利100%清仓**：持仓盈利>=100%全仓卖出
5. **半仓持股模式**（`hold_until_below_white=True`）：连续止盈后仓位<=initial_size/2进入此模式
   - 盈利<=20%：盈转亏清仓
   - 盈利>20%：跌破白线清仓
6. **涨停卖1/2**：仅中阳未触发时；剩余<=半仓进入条件5
7. **中阳卖1/3**：当日收盘价 > 前一日收盘价（`daily_up`）且涨幅>=主板5%/科创板10%；通过 `mid_yang_triggered` 标记确保每次持仓仅触发一次（避免中阳持续期间每天重复卖出）；触发后标记 `mid_yang_triggered=True`；剩余<=半仓进入条件5
