...
3. **突然放巨量阴线过滤**（随 N型B1 导入）：60日内出现 V>REF(V,1)*3 & V>MA(V,20)*3 & C<O & (O-C)/O>3% 则剔除（`no_huge_vol_bearish`）
4. **金叉+B1模式**（`_find_gc_b1_pattern()`）：白线在距今30bar内上穿黄线（金叉），金叉之后出现B1信号。回看窗口60天
5. **8项假案例排除**：快速上涨横盘派发、不规则上涨、B1死叉、跳空横盘出货、S1高位放量、不连续上涨、阶梯式量价背离、B1前跌停
...
- **突然放巨量阴线**过滤通过 N型B1 的 `no_huge_vol_bearish` 字段传递，金叉B1 无独立实现
