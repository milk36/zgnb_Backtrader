...
双引擎串行过滤策略：先由动能评分引擎（KDJ/RSI动量、Z-Score、筹码流量、综合天命打分）选出股票池，再在动能池中由金砖共振引擎（砖型图、绿转强红、黄柱动能）进一步筛选。最终股票按金砖排名"下大上小"取前N只，T+1开盘买入，四级退出（止损→涨停清仓→T+N不拉升→盈利止盈）。

## 2. How it Works

### 选股流程（串行过滤）

先动能筛选，再在动能池中金砖筛选（AND 逻辑，非并集）：
