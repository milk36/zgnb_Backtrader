...
6. **量能上下文** (`vol_context`): 根据前期走势二选一：
   - **前期大涨横盘放量** (`high_gain_consol`): 120日内涨幅>100% 且 近20日振幅<20%(横盘)时，要求红砖当日放量(V > 前日×1.5)
   - **其他情况**: 要求红砖前一日缩量(`prev_shrink`: V < HHV(V,20)×0.416 或 V < HHV(V,50)/3)
7. **砖块质量** (`brick_quality`): 红砖必须同时满足砖大(brick>3)、涨幅小(0<pct_chg<5%)、量能大(V>MA(V,5))
8. **买入条件**: 共振 AND 趋势条件 AND 上影线条件 AND 换手 AND 非涨停 AND vol_context AND brick_quality
