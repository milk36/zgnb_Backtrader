...
- [数据层：通达信数据馈送](feature/data-tdx-feed.md): TdxDataFeed 类的设计，mootdx Reader 封装，前复权调整模块（akshare + parquet 三级缓存），数据标准化流程。涉及数据获取、前复权配置与处理时参考。
...
- [策略层：黄白线B1策略 V4](feature/strategy-huangbai-b1-v4.md): V2 的变体版本，移除黄白线金叉条件放宽选股范围。保留大盘MACD过滤、周线多头、B1七子条件、vol_expand_ok过滤链和动量持股逻辑。`recent_gc` 返回全True数组以兼容 PortfolioSimulator。
